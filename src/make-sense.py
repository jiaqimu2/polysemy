...
if __name__ == "__main__":
	...
	semEvalFile = '/projects/csl/viswanath/data/jiaqimu2/wordnet/data/princeton/wordList/'
	...
	for pcaRank in [3,4,5]:
		for maxSenNum in [5, 2, 10]:
			...
			fout = open('/projects/csl/viswanath/data/jiaqimu2/wordnet/data/princeton/results/senNum-%d-pcaRank-%d.key' % (maxSenNum, pcaRank), 'w')
	...
	for pcaRank in [3,4,5]:
		...
		fout = open('/projects/csl/viswanath/data/jiaqimu2/wordnet/data/princeton/results/global-pcaRank-%d.key' % (pcaRank), 'w')
		for fname in os.listdir(semEvalFile):
			f = open(semEvalFile + fname, 'r')
			polysemy = fname.split('.')[0]
			if os.path.isfile(vecDir + polysemy + '.bin'):
